...
import { 
  Calendar, Users, DollarSign, Stethoscope, Activity, TrendingUp,
  Search, Filter, Star, FileText, Settings, User, Shield,
  Loader, Edit, Trash2, Eye, CheckCircle, XCircle, Clock
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (tab && ['overview', 'users', 'doctors', 'patients', 'appointments', 'reports', 'settings'].includes(tab)) {
      setActiveTab(tab);
    }
  }, [location.search]);
  ...
  try {
    ...
    // Fetch stats
    const statsRes = await axios.get(`${baseUrl}/stats?userId=${userId}`, config);
    ...
    // Fetch users
    const usersRes = await axios.get(`${baseUrl}/users?userId=${userId}`, config);
    ...
    // Fetch appointments
    const appointmentsRes = await axios.get(`${baseUrl}/appointments?userId=${userId}`, config);
    ...
    <div className="min-h-screen bg-gray-50 pt-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 mb-8 text-white">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-blue-100">Here's what's happening with your healthcare platform today.</p>
                  <div className="flex gap-4 mt-4">
                    ...
                    <div className="bg-white/20 rounded-lg px-4 py-2">
                      ...
                      <p className="text-2xl font-bold">{stats.totalUsers || stats.patients + stats.doctors}</p>
                    </div>
                  </div>
                </div>
                    ...
                    <div key={apt.id} className="p-4 hover:bg-gray-50 transition">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">with Dr. {apt.doctor_name}</p>
        ...
        {activeTab === 'users' && (
          ...
          >
            <div className="px-6 py-4 border-b flex flex-wrap justify-between items-center gap-4">
              <h2 className="text-lg font-bold text-gray-800">All Users ({filteredUsers.length})</h2>
        ...
        {activeTab === 'doctors' && (
          ...
          >
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-bold text-gray-800">All Doctors ({allDoctors.length})</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
              {allDoctors.length === 0 ? (
                <div className="col-span-3 text-center text-gray-500 py-12">No doctors found</div>
              ) : (
                allDoctors.map((doctor) => (
                  <div key={doctor.id} className="border rounded-xl p-4 hover:shadow-md transition">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
                        <span className="text-blue-600 font-bold text-lg">{doctor.name?.charAt(0)}</span>
                      </div>
                      <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <p className="text-gray-600">Phone: {doctor.phone || 'Not provided'}</p>
                      <p className="text-gray-600">City: {doctor.city || 'Not provided'}</p>
                    </div>
                  </div>
                ))
              )}
        ...
        {activeTab === 'patients' && (
          ...
          >
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-bold text-gray-800">All Patients ({allPatients.length})</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
              {allPatients.length === 0 ? (
                <div className="col-span-3 text-center text-gray-500 py-12">No patients found</div>
              ) : (
                allPatients.map((patient) => (
                  <div key={patient.id} className="border rounded-xl p-4 hover:shadow-md transition">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center">
                        <span className="text-green-600 font-bold text-lg">{patient.name?.charAt(0)}</span>
                      </div>
                      <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <p className="text-gray-600">Phone: {patient.phone || 'Not provided'}</p>
                      <p className="text-gray-600">City: {patient.city || 'Not provided'}</p>
                    </div>
                  </div>
                ))
              )}
        ...
        {activeTab === 'appointments' && (
          ...
          >
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-bold text-gray-800">All Appointments ({allAppointments.length})</h2>
            </div>
            <div className="divide-y divide-gray-100">
                  ...
                  <div key={apt.id} className="p-5 hover:bg-gray-50 transition">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        {apt.symptoms && (
                          <p className="text-sm text-gray-500 mt-2">Symptoms: {apt.symptoms}</p>
                        )}
                      </div>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadge(apt.status)}`}>
                        {apt.status}
        ...
        {activeTab === 'reports' && (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">Medical Reports</h3>
            <p className="text-gray-500">Total Reports: {stats.reports}</p>
            <p className="text-sm text-gray-400 mt-4">Coming soon: Detailed reports management</p>
          </motion.div>
        )}
              ...
              <div className="divide-y divide-gray-100">
                <div className="p-6 flex justify-between items-center hover:bg-gray-50 transition">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Put website under maintenance</p>
                  </div>
