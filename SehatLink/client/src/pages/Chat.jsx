...
const Chat = () => {
  ...
  const createConversation = async (patientId, doctorId) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/chat/conversation/${patientId}/${doctorId}`,
      );
  ...
  useEffect(() => {
    ...
    socketRef.current.on("newMessage", (message) => {
      if (
        ...
        fetchContacts();
        toast.success(`Message sent from ${message.senderName || "someone"}`);
      }
    });
    ...
    try {
      ...
      if (user?.role === "doctor") {
        response = await axios.get(
          `http://localhost:5000/api/chat/doctor-patients/${user.id}`,
        );
        const patients = response.data.patients || [];
        setContacts(patients);
      } else {
        response = await axios.get(
          `http://localhost:5000/api/chat/patient-doctors/${user.id}`,
        );
  ...
  const markMessagesAsRead = async (conversationId) => {
    try {
      await axios.put(
        `http://localhost:5000/api/chat/messages/read/${conversationId}`,
        {
          userId: user.id,
        },
      );
  ...
  const sendFile = async (file) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/chat/message/file",
        formData,
        {
                      ...
                      {contact.total_appointments && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <p className="text-xs text-gray-400">
                            {contact.total_appointments} appointments
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div key={date}>
                    ...
                    <AnimatePresence>
                      {dateMessages.map((message, idx) => (
                        ...
                        >
                          <div
                            className={`max-w-[70%] ${message.senderId === user.id ? "order-2" : "order-1"}`}
                          >
                            {message.hasFile ? (
                              <motion.div
