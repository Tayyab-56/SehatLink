...
import { 
  ...
  XCircle, DollarSign, Upload, Loader
} from 'lucide-react';
...
const Profile = () => {
  ...
  const validateForm = () => {
    ...
    newErrors.emergencyPhone = validateEmergencyPhone(profileData.emergencyPhone);
    newErrors.emergencyContact = validateEmergencyContact(profileData.emergencyContact);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/auth/profile/${userData.id}`, profileData);
  ...
  const isValid = (fieldName) => {
    return touched[fieldName] && profileData[fieldName] && (!errors[fieldName] || errors[fieldName] === '');
  };
...
const fetchCompleteProfile = async () => {
  try {
    ...
    if (userData && userData.id) {
      const response = await axios.get(`http://localhost:5000/api/auth/complete-profile/${userData.id}`);
    ...
    try {
      ...
      if (userData) {
        ...
        setStats({
          ...
          memberSince: new Date(userData.createdAt || Date.now()).toLocaleDateString('en-PK', { year: 'numeric', month: 'long' })
        });
      }
    } catch (error) {
  ...
  const statCards = [
    { title: 'Total Appointments', value: stats.appointments, icon: Calendar, color: 'blue', change: '+12%' },
    { title: 'Completed', value: stats.completed, icon: CheckCircle, color: 'green', change: '+8%' },
    { title: 'Cancelled', value: stats.cancelled, icon: XCircle, color: 'red', change: '-2%' },
    { title: 'Total Spent', value: `Rs. ${stats.totalSpent.toLocaleString()}`, icon: DollarSign, color: 'purple', change: '+15%' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-gray-500 mt-1">Manage your personal information and health data</p>
        </motion.div>
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden sticky top-24">
              ...
              <div className="relative px-6 pb-6">
                <div className="absolute -top-12 left-6">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition flex items-center justify-center cursor-pointer"
                         onClick={() => fileInputRef.current?.click()}>
              ...
              <div className="border-t px-6 py-4 space-y-3">
                ...
                <div className="flex items-center gap-3 text-gray-600">
                  ...
                  <span className="text-sm">{profileData.city || 'Not provided'}</span>
                </div>
                <div className="flex items-center gap-3 text-gray-600">
              ...
              <div className="bg-gray-50 p-6 rounded-b-2xl">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">{profileData.bloodGroup || 'Not specified'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Allergies</span>
                    <span className="font-medium">{profileData.allergies || 'None'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Emergency Contact</span>
                    <span className="font-medium">{profileData.emergencyContact || 'Not set'}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              {isEditing ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {Object.values(errors).some(e => e && e !== '') && (
                    <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                      ...
                      <ul className="text-red-500 text-xs space-y-1">
                        ...
                        {errors.emergencyPhone && <li>• {errors.emergencyPhone}</li>}
                        {errors.emergencyContact && <li>• {errors.emergencyContact}</li>}
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${
                          hasError('phone') ? 'border-red-500 bg-red-50' : isValid('phone') ? 'border-green-500' : 'border-gray-200'
                        }`}
                      />
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onBlur={handleBlur}
                        className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition ${
                          hasError('bloodGroup') ? 'border-red-500 bg-red-50' : isValid('bloodGroup') ? 'border-green-500' : 'border-gray-200'
                        }`}
                      >
                      ...
                      {hasError('bloodGroup') && <p className="text-red-500 text-xs mt-1">{errors.bloodGroup}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onBlur={handleBlur}
                        className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition ${
                          hasError('dateOfBirth') ? 'border-red-500 bg-red-50' : isValid('dateOfBirth') ? 'border-green-500' : 'border-gray-200'
                        }`}
                      />
                      {hasError('dateOfBirth') && <p className="text-red-500 text-xs mt-1">{errors.dateOfBirth}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition ${
                          hasError('emergencyContact') ? 'border-red-500 bg-red-50' : isValid('emergencyContact') ? 'border-green-500' : 'border-gray-200'
                        }`}
                      />
                      {hasError('emergencyContact') && <p className="text-red-500 text-xs mt-1">{errors.emergencyContact}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition ${
                          hasError('emergencyPhone') ? 'border-red-500 bg-red-50' : isValid('emergencyPhone') ? 'border-green-500' : 'border-gray-200'
                        }`}
                      />
                      {hasError('emergencyPhone') && <p className="text-red-500 text-xs mt-1">{errors.emergencyPhone}</p>}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition ${
                        hasError('allergies') ? 'border-red-500 bg-red-50' : isValid('allergies') ? 'border-green-500' : 'border-gray-200'
                      }`}
                    />
                    {hasError('allergies') && <p className="text-red-500 text-xs mt-1">{errors.allergies}</p>}
                  </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-gray-800">{profileData.bloodGroup || 'Not specified'}</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-500">Allergies</p>
                    <p className="text-gray-800">{profileData.allergies || 'None'}</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-500">Date of Birth</p>
                    <p className="text-gray-800">{profileData.dateOfBirth || 'Not provided'}</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-500">Emergency Contact</p>
                    <p className="text-gray-800">{profileData.emergencyContact || 'Not set'}</p>
                  </div>
                </div>
              )}
            </motion.div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-gray-600">Last login: Today at {new Date().toLocaleTimeString()}</p>
                  </div>
