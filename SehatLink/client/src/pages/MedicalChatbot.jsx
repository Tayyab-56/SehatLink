...
import { 
  Send, Bot, User, Loader, BrainCircuit, ArrowLeft, Sparkles, 
  Heart, Activity, Shield, Clock, MessageCircle, Menu, X, Trash2, Plus, History
} from 'lucide-react';
...
const loadConversation = async (convId) => {
  try {
    ...
    const response = await axios.get(`http://localhost:5000/api/chatbot/conversation/${convId}`, {
...
const deleteConversation = async (convId) => {
  if (window.confirm('Delete this conversation?')) {
    try {
      ...
      await axios.delete(`http://localhost:5000/api/chatbot/conversation/${convId}`, {
        data: { userId: userId },
        headers: { 'x-user-id': userId }
      });
...
const newConversation = async () => {
  try {
    ...
    const response = await axios.post('http://localhost:5000/api/chatbot/new-conversation', {
      userId: userId
    }, {
      headers: { 'x-user-id': userId }
    });
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-blue-100 text-xs">Powered by Medical Knowledge Graph</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-t bg-white p-4 flex-shrink-0">
            ...
            <div className="flex items-center justify-center gap-2 mt-2 text-xs text-gray-400">
              <Shield size={10} />
              <span>AI assistant - Not a doctor. Consult a professional for medical advice.</span>
            </div>
          </div>
        </div>
      </div>
