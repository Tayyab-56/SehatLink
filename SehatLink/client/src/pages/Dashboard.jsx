...
import { 
  Calendar, Stethoscope, DollarSign, Activity, Clock, MapPin,
  TrendingUp, Award, Heart, Shield, Star, ChevronRight,
  Sparkles, Video, MessageCircle, FileText, Bell, Users
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return <span className={`px-2 py-1 rounded-full text-xs font-medium ${styles[status] || styles.pending} shadow-sm`}>{status}</span>;
  };
  ...
  const statsCards = [
    { title: 'Total Appointments', value: stats.total, icon: Calendar, gradient: 'from-blue-500 to-cyan-500', change: '+12%', color: 'blue' },
    { title: 'Completed', value: stats.completed, icon: Activity, gradient: 'from-green-500 to-emerald-500', change: '+8%', color: 'green' },
    { title: 'Pending', value: stats.pending, icon: Clock, gradient: 'from-yellow-500 to-orange-500', change: '+5%', color: 'yellow' },
    { title: 'Total Spent', value: `Rs. ${stats.revenue.toLocaleString()}`, icon: DollarSign, gradient: 'from-purple-500 to-pink-500', change: '+15%', color: 'purple' },
  ];
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex items-center gap-3">
                  <div className="bg-white/20 backdrop-blur rounded-full px-4 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-white text-sm font-medium">Health Score: 92</span>
                    </div>
                  </div>
        ...
        >
          {statsCards.map((stat, index) => (
            ...
            >
              <div className={`absolute inset-0 bg-gradient-to-r ${stat.gradient} rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition duration-500`}></div>
              <div className="relative bg-white rounded-2xl shadow-lg p-6 overflow-hidden">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className={`w-12 h-12 bg-gradient-to-r ${stat.gradient} rounded-xl flex items-center justify-center shadow-lg group-hover:scale-110 transition duration-300`}>
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: Stethoscope, label: 'Book Doctor', color: 'from-blue-500 to-cyan-500', path: '/doctors' },
              { icon: Video, label: 'Video Consult', color: 'from-purple-500 to-pink-500', path: '/consult' },
              { icon: FileText, label: 'Upload Report', color: 'from-green-500 to-emerald-500', path: '/reports' },
              { icon: MessageCircle, label: 'Chat Support', color: 'from-orange-500 to-red-500', path: '/support' },
            ].map((action, idx) => (
              <Link key={idx} to={action.path}>
                ...
                >
                  <div className={`w-12 h-12 bg-gradient-to-r ${action.color} rounded-xl flex items-center justify-center mx-auto mb-3 shadow-md group-hover:shadow-lg transition`}>
        ...
        >
          <div className="px-6 py-4 border-b bg-gradient-to-r from-gray-50 to-white flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">Your upcoming and past consultations</p>
            </div>
            <Link to="/appointments" className="text-blue-600 text-sm font-medium hover:text-blue-700 flex items-center gap-1 group">
              View All
          ...
          <div className="divide-y divide-gray-100">
            {appointments.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <Link to="/doctors" className="inline-block mt-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-2 rounded-lg font-medium hover:shadow-lg transition">
                  Find a Doctor
                </Link>
              </div>
            ) : (
                ...
                >
                  <div className="flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-bold text-gray-800 mt-2">Rs. {apt.amount}</p>
                    </div>
                  </div>
                </motion.div>
              ))
            ...
            <div className="px-6 py-5 relative">
              ...
              <div className="relative flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-white font-bold text-lg">Health Tip of the Day</h3>
                    <p className="text-indigo-100 text-sm">Stay hydrated! Drink at least 8 glasses of water daily for optimal health.</p>
                  </div>
                </div>
